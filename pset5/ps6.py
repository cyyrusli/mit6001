...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(abcSize):
            dictionary[lower[i]] = lower[(i + shift) % abcSize]
            dictionary[upper[i]] = upper[(i + shift) % abcSize]
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            decryptedText = self.apply_shift(s)
            words = decryptedText.split()
            realWords = sum([is_word(self.get_valid_words(), w) for w in words])
            if realWords > maxWords:
                bestShift = s
                maxWords = realWords
                msg = decryptedText

        return (bestShift, msg)
